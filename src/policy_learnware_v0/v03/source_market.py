...
SOURCE_ATTESTATION_PLAN_SCHEMA = "policy-learnware.v03-source-attestation-plan.v0"
...
@dataclass(frozen=True)
class SourceAttestationPlan:
    intake_record_digest: str
    source_evaluation_protocol_digest: str
    provisional_selection_digest: str
    units: Mapping[str, SourceEvaluationWorkUnit]
    attestation_plan_digest: str | None = None
    schema: str = SOURCE_ATTESTATION_PLAN_SCHEMA

    def __post_init__(self) -> None:
        if self.schema != SOURCE_ATTESTATION_PLAN_SCHEMA:
            raise SourceMarketError("unsupported SourceAttestationPlan schema")
        for name in (
            "intake_record_digest",
            "source_evaluation_protocol_digest",
        ...
        if (
            len(units) != EXPECTED_ANCHOR_COUNT
            or any(not isinstance(unit, SourceEvaluationWorkUnit) for unit in units.values())
            or set(units) != {unit.candidate_id for unit in units.values()}
            or any(unit.block != "source_attestation" for unit in units.values())
        ):
            raise SourceMarketError("attestation plan must contain 30 keyed attestation work units")
        object.__setattr__(self, "units", MappingProxyType(dict(sorted(units.items()))))
        expected = sha256_json(self._payload_without_digest())
        if self.attestation_plan_digest is None:
            object.__setattr__(self, "attestation_plan_digest", expected)
        elif _digest(self.attestation_plan_digest, "attestation_plan_digest") != expected:
            raise SourceMarketError("source attestation-plan digest does not match contents")
    ...
    def _payload_without_digest(self) -> dict[str, Any]:
        return {
            ...
            "source_evaluation_protocol_digest": self.source_evaluation_protocol_digest,
            "provisional_selection_digest": self.provisional_selection_digest,
            "units": {candidate: unit.to_dict() for candidate, unit in self.units.items()},
        }

    def to_dict(self) -> dict[str, Any]:
        return {**self._payload_without_digest(), "attestation_plan_digest": self.attestation_plan_digest}

    @classmethod
    def from_dict(cls, value: Mapping[str, Any]) -> "SourceAttestationPlan":
        fields = set(cls.__dataclass_fields__)
        _strict(value, fields, "SourceAttestationPlan")
        units = value["units"]
        if not isinstance(units, Mapping):
            raise SourceMarketError("SourceAttestationPlan.units must be a mapping")
        return cls(
            **{
                name: (
                    {
                        candidate: SourceEvaluationWorkUnit.from_dict(unit)
...
def freeze_source_attestation_plan(
    intake: V03PoolIntakeRecord,
    protocol: SourceEvaluationProtocol,
    provisional: ProvisionalSourceSelection,
    attestation_work_units: Mapping[str, SourceEvaluationWorkUnit],
) -> SourceAttestationPlan:
    """Freeze the 30 winner-only attestation units before their rollouts."""

    if not isinstance(provisional, ProvisionalSourceSelection):
        raise SourceMarketError("attestation planning requires typed provisional selection")
    if (
        ...
        raise SourceMarketError("provisional selection differs from intake/protocol")
    units = dict(attestation_work_units)
    candidates = set(provisional.selected_candidate_ids.values())
    if len(units) != EXPECTED_ANCHOR_COUNT or set(units) != candidates:
        raise SourceMarketError("attestation work units must cover exactly 30 provisional winners")
    for candidate, unit in units.items():
        _bind_work_unit(unit, intake.cells[candidate], protocol, "source_attestation")
        anchor = intake.cells[candidate].source_anchor_id
        if provisional.selected_candidate_ids.get(anchor) != candidate:
            raise SourceMarketError("attestation work unit is not a provisional winner")
    return SourceAttestationPlan(
        intake_record_digest=intake.intake_record_digest,
        source_evaluation_protocol_digest=protocol.source_evaluation_protocol_digest,
        provisional_selection_digest=provisional.provisional_selection_digest,
        units=units,
    )


def finalize_source_championization(
    intake: V03PoolIntakeRecord,
    protocol: SourceEvaluationProtocol,
    provisional: ProvisionalSourceSelection,
    attestation_plan: SourceAttestationPlan,
    attestation_receipts: Sequence[EvaluatorSourceReceipt],
) -> SourceChampionizationRecord:
    """Finalize champions only against a pre-outcome attestation plan."""

    if not isinstance(provisional, ProvisionalSourceSelection):
        raise SourceMarketError("finalization requires typed provisional selection")
    if not isinstance(attestation_plan, SourceAttestationPlan):
        raise SourceMarketError("finalization requires a frozen source-attestation plan")
    if (
        provisional.intake_record_digest != intake.intake_record_digest
        or provisional.source_evaluation_protocol_digest
        != protocol.source_evaluation_protocol_digest
        or attestation_plan.intake_record_digest != intake.intake_record_digest
        or attestation_plan.source_evaluation_protocol_digest
        != protocol.source_evaluation_protocol_digest
        or attestation_plan.provisional_selection_digest
        != provisional.provisional_selection_digest
    ):
        raise SourceMarketError("championization stage digests do not form one lineage")
    attestation = _index_receipts(attestation_receipts, "source_attestation")
    winner_ids = set(provisional.selected_candidate_ids.values())
    if len(attestation) != EXPECTED_ANCHOR_COUNT or set(attestation) != winner_ids:
        raise SourceMarketError("attestation receipts must cover every provisional winner once")
    for candidate, receipt in attestation.items():
        cell = intake.cells[candidate]
        unit = attestation_plan.units[candidate]
        _bind_work_unit(unit, cell, protocol, "source_attestation")
        _bind_receipt(receipt, cell, protocol)
        if (
            receipt.work_unit_digest != unit.work_unit_digest
            or receipt.runtime_digest != unit.anchor_runtime_digest
        ):
            raise SourceMarketError("attestation receipt belongs to another frozen work unit")
    if len({receipt.dataset_digest for receipt in attestation.values()}) != EXPECTED_ANCHOR_COUNT:
        raise SourceMarketError("each champion attestation requires its own dataset")
    attestation_index_digest = sha256_json(
        {
            "schema": "policy-learnware.v03-source-attestation-receipt-index.v0",
            "attestation_plan_digest": attestation_plan.attestation_plan_digest,
            "receipts": {
                candidate: receipt.receipt_digest
                for candidate, receipt in sorted(attestation.items())
            },
        }
    )
    ...
    for anchor, candidate in provisional.selected_candidate_ids.items():
        cell = intake.cells[candidate]
        receipt = attestation[candidate]
        lcb = float(
            receipt.mean - protocol.lcb_z * receipt.std / math.sqrt(receipt.episode_count)
        )
        ...
        champions[anchor] = SourceChampion(
            ...
            outer_iteration=cell.outer_iteration,
            environment_steps=cell.environment_steps,
            selection_receipt_digest=provisional.selected_receipt_digests[anchor],
            attestation_receipt_digest=receipt.receipt_digest,
    ...
    return SourceChampionizationRecord(
        ...
        attestation_receipt_index_digest=attestation_index_digest,
        champions=champions,
        provisional_selection_digest=provisional.provisional_selection_digest,
        attestation_plan_digest=attestation_plan.attestation_plan_digest,
    )
...
def championize_source_pool(
    intake: V03PoolIntakeRecord,
    protocol: SourceEvaluationProtocol,
    selection_receipts: Sequence[EvaluatorSourceReceipt],
    attestation_receipts: Sequence[EvaluatorSourceReceipt],
) -> SourceChampionizationRecord:
    """Legacy in-memory convenience path for source-only fixtures.

    Formal P5M execution must use ``provisionally_select_source_pool``, freeze a
    ``SourceAttestationPlan`` before attestation outcomes exist, and then call
    ``finalize_source_championization``.
    """

    if not isinstance(intake, V03PoolIntakeRecord) or intake.pool_state != "POOL_READY":
        raise SourceMarketError("source championization requires a typed POOL_READY intake")
    if not isinstance(protocol, SourceEvaluationProtocol):
        raise SourceMarketError("source championization requires a typed evaluator protocol")
    if protocol.intake_record_digest != intake.intake_record_digest:
        raise SourceMarketError("source evaluator protocol is bound to another intake")
    if set(protocol.source_environment_digests) != set(intake.candidates_by_anchor):
        raise SourceMarketError("source evaluator protocol anchor set differs from intake")

    selection = _index_receipts(selection_receipts, "source_selection")
    if len(selection) != EXPECTED_JOB_COUNT or set(selection) != set(intake.cells):
        raise SourceMarketError("selection receipts must cover all exact-90 candidates once")
    for candidate, receipt in selection.items():
        _bind_receipt(receipt, intake.cells[candidate], protocol)
    selection_seed_banks = {receipt.reset_seeds for receipt in selection.values()}
    if len(selection_seed_banks) != 1:
        raise SourceMarketError("all candidates must use the same source-selection seed bank")

    selected: dict[str, PoolIntakeCell] = {}
    for anchor, candidates in intake.candidates_by_anchor.items():
        if len(candidates) != 3:
            raise SourceMarketError("each source anchor must have exactly three candidates")
        rows = [(cell, selection[cell.job_id]) for cell in candidates]
        best_mean = max(receipt.mean for _, receipt in rows)
        tied = [
            (cell, receipt)
            for cell, receipt in rows
    ...
    attestation = _index_receipts(attestation_receipts, "source_attestation")
    winner_ids = {cell.job_id for cell in selected.values()}
    if len(attestation) != EXPECTED_ANCHOR_COUNT or set(attestation) != winner_ids:
        raise SourceMarketError("attestation receipts must cover only every selected champion")
    for candidate, receipt in attestation.items():
        _bind_receipt(receipt, intake.cells[candidate], protocol)
    attestation_seed_banks = {receipt.reset_seeds for receipt in attestation.values()}
    if len(attestation_seed_banks) != 1:
        raise SourceMarketError("all champions must use the same source-attestation seed bank")
    if set(next(iter(selection_seed_banks))) & set(next(iter(attestation_seed_banks))):
        raise SourceMarketError("source selection and attestation episode seeds overlap")
    if len({receipt.dataset_digest for receipt in selection.values()}) != EXPECTED_JOB_COUNT:
        raise SourceMarketError("each candidate selection receipt requires its own dataset")
    if len({receipt.dataset_digest for receipt in attestation.values()}) != EXPECTED_ANCHOR_COUNT:
        raise SourceMarketError("each champion attestation requires its own dataset")
    ...
    selection_index_digest = sha256_json(
        {
            "schema": "policy-learnware.v03-source-selection-receipt-index.v0",
            "receipts": {
                candidate: receipt.receipt_digest for candidate, receipt in sorted(selection.items())
            },
        }
    )
    attestation_index_digest = sha256_json(
    ...
    champions: dict[str, SourceChampion] = {}
    for anchor, cell in sorted(selected.items()):
        receipt = attestation[cell.job_id]
        lcb = float(
            receipt.mean
        ...
        champions[anchor] = SourceChampion(
            ...
            outer_iteration=cell.outer_iteration,
            environment_steps=cell.environment_steps,
            selection_receipt_digest=selection[cell.job_id].receipt_digest,
            attestation_receipt_digest=receipt.receipt_digest,
            competence=competence,
        )
    return SourceChampionizationRecord(
